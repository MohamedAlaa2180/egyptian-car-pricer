...
Put `HF_TOKEN` in `.env` if the Hub rate-limits anonymous downloads. **Do not commit `.env`.**
...
2. **Train (later)** — `notebooks/02_train_qlora_colab.ipynb` on Google Colab T4.  
   Model: `Qwen/Qwen2.5-3B` (base, not Instruct), QLoRA 4-bit NF4 + LoRA.
3. **Eval (later)** — `notebooks/03_eval.ipynb`  
   200-row held-out test, MAE + MAPE vs mean/median baselines and unadapted Qwen.

`LITE_MODE` uses 8,000 train rows so a T4 run is cheap. Full mode uses all remaining train rows. The **same 200 test cars** are kept in both modes.
